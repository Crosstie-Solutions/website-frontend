...
function CrossContextProvider(props) {
  ...
  //sort by index
  const allOpenPrograms =
    open && open.sort((a, b) => a.priorityIndex - b.priorityIndex);

  const filteredHeaderPrograms =
    allPrograms &&
    allPrograms.filter((program) =>
      `${program.title} ${program.category}`
        .toLowerCase()
  ...
  const [secondOpenEnrollmentForm, setSecondOpenEnrollmentForm] =
    useState(null);
  ...
  const executiveCourse =
    program && program.category === "Open Executive Programmes (OEP)";
  ...
  const upcomingWebinars =
    allWebinars && allWebinars.filter((webinar) => webinar.past === false);

  const pastWebinars =
    allWebinars && allWebinars.filter((webinar) => webinar.past === true);
  ...
  const totalUpcomingPages =
    filteredUpcoming && Math.ceil(filteredUpcoming.length / upcomingPerPage);
  ...
  const currentUpcoming =
    filteredUpcoming &&
    filteredUpcoming.slice(upcomingStartIndex, upcomingEndIndex).reverse();
  ...
  const totalPastPages =
    filteredPast && Math.ceil(filteredPast.length / pastPerPage);

  // Get requests for the current page
  const pastStartIndex = (currentPastPage - 1) * pastPerPage;
  const pastEndIndex = pastStartIndex + pastPerPage;
  const currentPast =
    filteredPast && filteredPast.slice(pastStartIndex, pastEndIndex).reverse();
  ...
  const deleteUser = async (userId) => {
    ...
    try {
      ...
      const response = await axios.delete(
        `${baseUrl}/api/users/delete-user/${userId && userId}`,
        {
          headers: {
            Authorization: `Bearer ${loginToken ? loginToken : ""}`,
          },
  ...
  // // Calculate total pages
  const totalCourseRegsPages =
    filteredCourseRegs &&
    Math.ceil(filteredCourseRegs.length / courseRegsPerPage);
  ...
  const currentCourseRegs =
    filteredCourseRegs &&
    filteredCourseRegs
      .slice(courseRegsStartIndex, courseRegsEndIndex)
      .reverse();
  ...
  const [loadingAllConsultingReqs, setLoadingAllConsultingReqs] =
    useState(false);
  ...
  // // Calculate total pages
  const totalConsultingReqsPages =
    filteredConsultingReqs &&
    Math.ceil(filteredConsultingReqs.length / consultingReqsPerPage);

  // Get requests for the current page
  const consultingReqsStartIndex =
    (currentConsultingReqsPage - 1) * consultingReqsPerPage;
  const consultingReqsEndIndex =
    consultingReqsStartIndex + consultingReqsPerPage;
  const currentConsultingReqs =
    filteredConsultingReqs &&
  ...
  // // Calculate total pages
  const totalEnquiriesPages =
    allEnquiries && Math.ceil(allEnquiries.length / enquiriesPerPage);
  ...
  const currentEnquiries =
    allEnquiries && allEnquiries.slice(enquiriesStartIndex, enquiriesEndIndex);
  ...
  // // Calculate total pages
  const totalDownloadsPages =
    allDownloads && Math.ceil(allDownloads.length / downloadsPerPage);
  ...
  const currentDownloads =
    allDownloads && allDownloads.slice(downloadsStartIndex, downloadsEndIndex);
  ...
  const allTestimonials =
    testimonials &&
    testimonials.sort((a, b) => a.priorityIndex - b.priorityIndex);
  ...
  const totalTestimonialsPages =
    allTestimonials && Math.ceil(allTestimonials.length / testimonialsPerPage);

  // Get requests for the current page
  const testimonialsStartIndex =
    (currentTestimonialsPage - 1) * testimonialsPerPage;
  const testimonialsEndIndex = testimonialsStartIndex + testimonialsPerPage;
  const currentTestimonials =
    allTestimonials &&
    allTestimonials.slice(testimonialsStartIndex, testimonialsEndIndex);
  ...
  // Calculate total pages
  const totalContactFormsPages =
    allContactForms && Math.ceil(allContactForms.length / contactFormsPerPage);

  // Get requests for the current page
  const contactFormsStartIndex =
    (currentContactFormsPage - 1) * contactFormsPerPage;
  const contactFormsEndIndex = contactFormsStartIndex + contactFormsPerPage;
  const currentContactForms =
    allContactForms &&
    allContactForms.slice(contactFormsStartIndex, contactFormsEndIndex);
  ...
  //downloadScreens
  const [presentationDownloadScreen, setPresentationDownloadScreen] =
    useState(false);
  ...
  const [courseContentDownloadScreen, setCourseContentDownloadScreen] =
    useState(false);
  ...
  const [courseBrochureDownloadScreen, setCourseBrochureDownloadScreen] =
    useState(false);
  ...
  const downloadUrl =
    "https://res.cloudinary.com/dnq0mhrjs/image/upload/v1744802770/Crosstie_Corporate_Profile_2025_sap8fs.pdf";

  const title = "Our Corporate Presentation";

  //webinar certificate template
  const webinarCertTemplate =
    "https://res.cloudinary.com/dnq0mhrjs/image/upload/v1748066017/Complete_Webinar_Certificate_4_tutmq2.pdf";
  ...
  // // Calculate total pages
  const totalBlogPostsPages =
    filteredBlogPosts && Math.ceil(filteredBlogPosts.length / blogPostsPerPage);
  ...
  const currentBlogPosts =
    filteredBlogPosts &&
    filteredBlogPosts.slice(blogPostsStartIndex, blogPostsEndIndex).reverse();
  ...
  // // Calculate total pages
  const totalReportsPages =
    filteredReports && Math.ceil(filteredReports.length / reportsPerPage);
  ...
  const currentReports =
    filteredReports &&
    filteredReports.slice(reportsStartIndex, reportsEndIndex).reverse();
  ...
  const totalUsersPages =
    filteredUsers && Math.ceil(filteredUsers.length / usersPerPage);
  ...
  const currentUsers =
    filteredUsers &&
    filteredUsers.slice(usersStartIndex, usersEndIndex).reverse();
  ...
  // Calculate total pages
  const totalPartnersPages =
    filteredPartners && Math.ceil(filteredPartners.length / partnersPerPage);
  ...
  const currentPartners =
    filteredPartners &&
    filteredPartners.slice(partnersStartIndex, partnersEndIndex).reverse();
  ...
  const allEvents =
    events && events.sort((a, b) => a.priorityIndex - b.priorityIndex);
  ...
  const totalEventsPages =
    filteredEvents && Math.ceil(filteredEvents.length / eventsPerPage);
  ...
  const currentEvents =
    filteredEvents &&
    filteredEvents.slice(eventsStartIndex, eventsEndIndex).reverse();
  ...
  const deleteEvent = async (eventId) => {
    try {
      ...
      const response = await axios.delete(`${baseUrl}/api/event/${eventId}`);
  ...
  //for admin to fetch all Members
  ...
  const allMembers =
    members && members.sort((a, b) => a.priorityIndex - b.priorityIndex);
  ...
  const totalMembersPages =
    filteredMembers && Math.ceil(filteredMembers.length / membersPerPage);

  // Get requests for the current page
  const membersStartIndex = (currentMembersPage - 1) * membersPerPage;
  const membersEndIndex = membersStartIndex + membersPerPage;
  const currentMembers =
    filteredMembers &&
    filteredMembers.slice(membersStartIndex, membersEndIndex).reverse();

  const handleMembersPageChange = (page) => {
    if (page > 0 && page <= totalMembersPages) {
  ...
  const toggleMember = async (index) => {
    window.scrollTo({ top: 0, behavior: "auto" });
  ...
  const deleteMember = async (memberId) => {
    try {
      ...
      const response = await axios.delete(`${baseUrl}/api/member/${memberId}`);
  ...
  const currentJobs =
    filteredJobs && filteredJobs.slice(jobsStartIndex, jobsEndIndex).reverse();
  ...
  const currentProducts =
    filteredProducts &&
    filteredProducts.slice(productsStartIndex, productsEndIndex);
  ...
  const [webinarFeedbackSearchTerm, setWebinarFeedbackSearchTerm] =
    useState("");

  // Filter WebinarFeedback based on search term
  const filteredWebinarFeedback =
    allWebinarFeedbacks &&
...
const skillsList = [
  ...
  const contextValue = {
    ...
    departments, jobModes, skillsList, jobTypes, experienceLevels,
    allTalents,
    currentTalents,
    ...
    allPrograms,
    formatDate,
    setProgramsSearchTerm,
    programsSearchTerm,
    setCurrentProgramsPage,
    currentPrograms,
    currentProgramsPage,
    totalProgramsPages,
    programsStartIndex,
    ...
    pastWebinars,
    activeWebinar,
    toggleActiveWebinarView,
    activeWebinarView,
    webinarEnrollment,
    toggleWebinarEnrollment,
    webinar,
    setWebinarEnrollment,
    loadingWebinar,
    viewAllEnquiries,
    ...
    currentBlogPosts,
    totalBlogPostsPages,
    setBlogPostsSearchTerm,
    toggleAdminBlogPostAction,
    currentTestimonialsPage,
    totalTestimonialsPages,
    loadingAllCaseStudies,
    currentCaseStudies,
    caseStudiesSearchTerm,
    setCaseStudiesSearchTerm,
    setCurrentCaseStudiesPage,
    ...
    openEnrollmentForm,
    executiveEnrollmentData,
    handleExecutiveChange,
    trainingMode,
    setTrainingMode,
    setExecutiveEnrollmentData,
    toggleSecondOpenEnrollment,
    secondOpenEnrollmentForm,
    currentEvents,
    allEvents,
    fetchEvents,
    loadEvents,
    togglePresentationDownloadScreen,
    toggleCourseContentDownloadScreen,
    presentationDownloadScreen,
    courseContentDownloadScreen,
    courseBrochureDownloadScreen,
    toggleCourseBrochureDownloadScreen,
    eventsSearchTerm,
    setEventsSearchTerm,
    ...
    loadMe,
    currentJobsPage,
    headerProgramsSearchTerm,
    setHeaderProgramsSearchTerm,
    totalJobsPages,
    currentTestimonials,
    handleTestimonialsPageChange,
    headerProgramsSearchTerm,
    ...
    deletingEvent,
    allWebinarFeedbacks,
    webinarFeedbackSearchTerm,
    setWebinarFeedbackSearchTerm,
