...
import { UpcomingFilter } from '../../WebinarFilter/WebinarFilter';
...
function UpcomingWebinarsTable() {
    ...
    const {
        ...
        currentUpcoming,
        activeWebinar, toggleAdminWebinarAction,
  ...
  return (
    <div className='flex flex-col items-center h-auto gap-2 w-100'>
      <div className='flex items-center h-auto gap-5 w-100'>
        <h4 className="self-start font-semibold text-crossLightPurple small:hidden large:block large:text-17px w-30">Upcoming Webinars({upcomingWebinars && upcomingWebinars.length})</h4>
            ...
            currentUpcoming && currentUpcoming.map((webinar, i)=>
                <div className={`flex items-center justify-between h-auto w-100 ${i % 2 === 0 ? "bg-gray-100" : "bg-white"} pl-1 py-1`}>
                    ...
                    <div>
                      <AdminWebinarAction 
                      webinarNo={i + 1}
                      webinarEnd={currentUpcoming.length + 1}
      ...
      {/* Pagination */}
      {!loadingAllWebinars && currentUpcoming && currentUpcoming.length > 0 && (
        <div className="flex items-center justify-between h-auto gap-3 mt-4 large:w-50 small:w-80">
          <button
            ...
            onClick={() => handleUpcomingPageChange(currentUpcomingPage - 1)}
          >
    ...
    {!loadingAllWebinars && currentUpcoming && currentUpcoming.length < 1 && (
        <p className="mt-5 text-center w-100 text-15px">No result found.</p>
      )}

        {loadingAllWebinars &&  (
        <p className="mt-5 text-center w-100 text-15px">Loading webinars.</p>
      )}
    </div>
  )
}

export default UpcomingWebinarsTable
