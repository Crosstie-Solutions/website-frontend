...
function AdminMemberAction(post) {

    const { postEnd, progNo, eventId, name, tier } = post;

    
    const { 

      activeMember, toggleMember, deleteMember, deletingMember
    } = useContext(CrossContext);


    //to delete report
    const [deleteMemberMode, setDeleteMemberMode] = useState(false)

    const toggleDeleteMemberMode = ()=>{
      setDeleteMemberMode(!deleteMemberMode);
    }
  ...
  return (
    <div className='fixed top-0 left-0 z-20 flex flex-col items-center justify-center h-100vh w-100vw bg-overlay'>
      ...
      <div className='flex flex-col items-center justify-center h-auto px-2 py-3 bg-white large:gap-3 large:w-30vw rounded-10 small:w-90vw small:gap-1'>
        ...
        {!deletingMember &&
          <h4 className='text-center'>Are you sure you want to delete <span className='font-semibold'>{name}</span> from <span className='font-semibold'>{tier}?</span></h4>}
...
export default AdminMemberAction;
