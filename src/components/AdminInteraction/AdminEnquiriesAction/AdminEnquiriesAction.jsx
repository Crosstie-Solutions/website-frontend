...
function AdminEnquiriesAction(enquiry) {

    const { enquiryNo, enquiryEnd, enquiryId, program, nameOfOrg, mode, participants,  location, preferredDate, designation, title, fullName, email, phone, message, duration, country, city, userType } = enquiry;
  ...
  return (
    <div className='absolute top-0 left-0 z-20 flex flex-col items-center justify-center h-auto py-10 w-100vw bg-overlay'>
        ...
        onClick={()=>toggleAdminEnquiryAction(enquiryEnd)}
        />
      ...
      <div className='flex flex-col items-center justify-center h-auto px-2 py-5 bg-white large:gap-3 large:w-60vw rounded-10 small:w-90vw small:gap-1'>
        ...
        <h2 className='text-20px'>Enquiry About - <span className='font-semibold'>{program}</span></h2>
       ...
       <div className='flex flex-col h-auto gap-1 large:w-90 small:w-100'>
        ...
        <div className='flex flex-col h-auto gap-1 w-100'>
          <div>Course: <span className='font-semibold'>{program}</span></div>
...
export default AdminEnquiriesAction;