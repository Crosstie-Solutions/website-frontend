...
const ButtonSwiper = () => {
  ...
  return (
    <div className="flex flex-row justify-center overflow-hidden border border-red-500 swiper-container large:w-100 large:text-15px small:w-90 small:text-10px">
      ...
      <div className="flex flex-row justify-center large:w-auto gap-2 px-1 py-0.5 border border-crossFooterText buttons-wrapper rounded-20 small:w-100" ref={scrollContainerRef}>
        {routes.map((item, index) => (
          <Link to={item.route} key={index} className="flex items-center justify-center w-auto large:px-2 swiper-button large:h-40px bg-crossLightPurple rounded-20 small:px-1 small:h-30px">{item.name}</Link>
        ))}
      </div>
