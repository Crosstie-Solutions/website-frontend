...
function EventsFilter() {
    ...
    const {
      eventsSearchTerm, setEventsSearchTerm, setCurrentEventsPage
  ...
  return (
    <div className="flex flex-row items-center gap-1 p-1 border rounded-10 h-100 w-100 text-15px">
        ...
        <input
          type="text"
          placeholder="Search by event and client name..."
          value={eventsSearchTerm}
          onChange={(e) => {
            setEventsSearchTerm(e.target.value);
            setCurrentEventsPage(1); // Reset to first page on search
          }}
