...
function PresentationDownloadScreen(props) {

    const {downloadUrl, title} = props;

    const [screen, setScreen] = useState("form");

   const {toggleDownloadScreen, baseUrl, setLoading,  setDownloadScreen, setDownloadProgramScreen, togglePresentationDownloadScreen} = useContext(CrossContext);
  ...
  return (
    <div className={`absolute left-0 z-20 flex flex-col items-center justify-center py-10 ${window.location.pathname.includes("about") || window.location.pathname.includes("solutions") || window.location.pathname.includes("courses") ? "top-0" : "-top-8"} min-h-100vh w-100vw bg-overlay`}>
        
        
        <VscClose className='absolute text-white cursor-pointer large:top-5 large:right-10 text-30px small:right-5 small:top-3'
        onClick={togglePresentationDownloadScreen}
        />
...
function CourseBrochureDownloadScreen(props) {

  const {downloadUrl, title} = props;

  const [screen, setScreen] = useState("form");

 const {toggleDownloadScreen, baseUrl, setLoading,  setDownloadScreen, setDownloadProgramScreen, toggleDownloadProgramScreen, toggleCourseBrochureDownloadScreen} = useContext(CrossContext);
...
return (
  <div className={`absolute left-0 z-20 flex flex-col items-center justify-center py-10 ${window.location.pathname.includes("about") || window.location.pathname.includes("solutions") || window.location.pathname.includes("courses") ? "-top-17" : "-top-8"} min-h-100vh w-100vw bg-overlay`}>
      ...
      <VscClose className='absolute text-white cursor-pointer large:top-5 large:right-10 text-30px small:right-5 small:top-3'
      onClick={toggleCourseBrochureDownloadScreen}
      />
...
function CourseContentDownloadScreen(props) {
 ...
 const {toggleDownloadScreen, baseUrl, setLoading,  setDownloadScreen, setDownloadProgramScreen, toggleDownloadProgramScreen} = useContext(CrossContext);
...
export {PresentationDownloadScreen, CourseContentDownloadScreen, CourseBrochureDownloadScreen};
