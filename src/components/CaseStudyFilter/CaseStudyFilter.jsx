...
function CaseStudyFilter() {
    ...
    const {
       caseStudiesSearchTerm, setCaseStudiesSearchTerm, setCurrentCaseStudiesPage
  ...
  return (
    <div className="flex flex-row items-center gap-1 p-1 border rounded-10 h-100 w-100 text-15px">
        ...
        <input
          type="text"
          placeholder="Search by title and name of author..."
          value={caseStudiesSearchTerm}
          onChange={(e) => {
            setCaseStudiesSearchTerm(e.target.value);
            setCurrentCaseStudiesPage(1); // Reset to first page on search
          }}
