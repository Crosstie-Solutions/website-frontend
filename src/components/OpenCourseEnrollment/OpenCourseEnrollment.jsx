...
function OpenCourseEnrollment() {
  const { toggleEnrollment, program, setLoading, baseUrl, executiveCourse, toggleOpenEnrollment} = useContext(CrossContext);
...
function SecondOpenCourseEnrollment() {
  ...
  const handleCourseEnrollment = async (event) => {
   ...
   //To ensure valid inputs
   if (!executiveEnrollmentData.fullName.trim()) {
     validationErrors.fullName = "full name is required";
   }
      ...
      <div className="relative flex flex-col items-center h-auto gap-3 py-5 mt-5 bg-white large:px-3 large:w-83vw rounded-10 small:w-90vw small:px-1">
        ...
        <div className="flex gap-2 font-semibold large:px-2 w-100 large:flex-row small:flex-col">
          ...
          <div className="flex items-center justify-start gap-1 border rounded h-30px px-0.5">
            ...
            <div>{executiveEnrollmentData.preferredDate}</div>
          </div>
...
export {OpenCourseEnrollment, SecondOpenCourseEnrollment};
