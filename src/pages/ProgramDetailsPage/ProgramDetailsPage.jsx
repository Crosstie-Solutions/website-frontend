...
function ProgramDetailsPage() {
  ...
  const executiveProgram =
    program && program.course.courseTitle === "Open Executive Programmes (OEP)";
  ...
  const whatsAppMessage = encodeURIComponent(
    `Hello Crosstie, I have questions about ${program && program.title}.`,
  );
    ...
    if (noError) {
      try {
        ...
        if (response.status === 201) {
          toast.success(
            "Enquiry form submitted successfully. Our team will reach out via email.",
          );
  ...
  const summary = program && splitParagraphByFullStops(program.description);
    ...
    <div className="relative flex flex-col items-center justify-start gap-5 large:mt-15 large:text-15px large:w-100vw large:h-auto small:w-100vw small:h-auto small:mt-12 small:text-13px">
      <div className="relative flex items-center text-white small:flex-col-reverse large:justify-center large:gap-2 large:w-100vw large:h-500px small:px-0 large:p-0 small:gap-1 small:h-auto small:w-100vw crossLightPurple bg-crossLightPurple large:flex-row small:py-2 large:py-0">
        ...
        <div className="flex flex-col justify-center gap-2 large:pl-0 large:h-500px large:w-50 small:w-100vw small:h-auto small:pl-2 bg-crossLightPurple small:py-2 large:py-0 large:relative large:-left-2">
          ...
          {!loadingProgram && (
            <div
              className="flex items-center justify-center gap-1 px-1 cursor-pointer large:w-300px rounded-10 h-40px bg-buttonOverlay large:text-15px small:w-250px small:text-11px"
              onClick={() => {
          ...
          {!loadingProgram && me && me.role !== "user" && (
            <div className="flex items-center w-auto h-auto gap-2">
              <Link
                to="/admin-dashboard"
                className="flex items-center justify-center w-auto gap-1 px-1 border rounded large:mt-3 small:mt-0 h-30px"
              >
                {" "}
                <LuMoveLeft className="flex text-25px" /> Admin dashboard
              </Link>
              ...
              >
                {" "}
                <CiEdit className="flex text-25px" /> Edit Course
              </Link>
            </div>
          )}
        </div>
        ...
        {!loadingProgram && (
          <img
            src={program && program.programBanner}
            alt="course banner"
            className="large:h-75 large:w-auto small:w-70vw small:h-auto large:relative large:-right-2 small:mt-3"
          />
        )}
      </div>
      ...
      {executiveProgram && (
        <div className="flex flex-col items-center pb-5 bg-white border large:gap-5 rounded-tl-20 large:w-83vw rounded-tr-20 small:w-90vw small:gap-3 small:mt-3 large:mt-0">
          <div className="flex items-center justify-center text-white rounded-tl-20 rounded-tr-20 large:h-50px w-100 bg-crossLightPurple small:h-auto large:text-20px small:text-13px large:flex-row small:flex-col small:text-center small:py-1">
            ...
            <span className="font-bold"> {program && program.title}</span>
          </div>
          ...
          <div className="flex flex-col items-start h-auto small:gap-1 large:gap-2 small:pl-1 large:pl-5 w-100 text-crossTextGray small:py-1 large:py-0">
            ...
            <div className="flex items-center justify-center w-auto h-auto gap-2">
              ...
              <div>
                ...
                <span className="text-black">
                  &#8358;{program && program.physicalCost.toLocaleString()}
                </span>{" "}
                Per Participant
              </div>
            </div>}
            ...
            <div className="flex items-center justify-center w-auto h-auto gap-2">
              ...
              <div>
                Online -{" "}
                <span className="text-black">
                  &#8358;{program && program.onlineCost.toLocaleString()}
                </span>{" "}
                Per Participant
              </div>
            </div>}

              {program && program.youAndICost !==0 &&
            <div className="flex items-center justify-center w-auto h-auto gap-2">
              <hr className="rotate-45 large:h-20px large:w-20px small:w-10px small:h-10px bg-crossLightPurple" />
              <div>
                You And I -{" "}
                <span className="text-black">
                  &#8358;{program && program.youAndICost.toLocaleString()}
                </span>{" "}
                Per Participant
              </div>
            </div>}
          ...
          <div className="flex flex-col h-auto gap-2 large:pl-5 w-100 small:px-1">
            ...
            <div className="flex flex-col h-auto gap-2 bg-white w-100">
              {program &&
                program.date.map((day, i) => (
                  <label
                    key={i}
                    className="flex large:w-50 small:w-100 h-auto option-label items-center justify-between large:px-2 py-2 pr-2 border border-crossFooterText bg-[#F9F9F9] flex-row small:px-1"
                  >
                    <input
                      type="radio"
                      name="preferredDate"
                      value={day}
                      checked={executiveEnrollmentData.preferredDate === day}
                    ...
                    <div className="flex items-center self-end justify-center w-auto large:gap-1">
            ...
            <div className="flex items-center justify-between large:pr-10 w-100 large:flex-row small:flex-col small:gap-2 large:gap-0">
              <Link
                to="/about-us/satisfaction"
                className="flex items-center w-auto h-auto gap-1 font-bold hover:text-crossLightPurple"
              >
                <CiFaceSmile className="text-20px text-crossYellow hover:text-crossYellow" />
                Satisfaction guaranteed?
              </Link>

              <button
                className="flex items-center justify-center w-auto px-2 text-white h-40px bg-crossLightPurple rounded-20"
                onClick={() => {
                  executiveProgram
                    ? toggleSecondOpenEnrollment(program && program.slug)
      ...
      <div className="flex justify-between large:items-start large:w-83vw small:flex-col large:flex-row small:w-90vw small:items-center small:gap-3 large:gap-0">
        <div className="flex flex-col gap-3 large:w-40 small:w-100 small:px-1 large:px-0 small:py-3 large:py-0">
          <div className="flex flex-col gap-1 bg-white w-100">
            <h4 className="flex gap-1 p-1 text-white bg-crossLightPurple">
              <MdOutlineDescription className="text-20px" /> About The Course
            </h4>
          ...
          <div className="flex flex-col items-center justify-center h-auto bg-white w-100">
            <h4 className="flex gap-1 p-1 text-white bg-crossLightPurple w-100">
              <MdOutlineViewModule className="text-20px" /> Modules
            </h4>
          ...
          {program && program.optionalModules[0] !== "" && (
            <div className="flex flex-col items-center justify-center h-auto bg-white w-100">
              <h4 className="flex gap-1 p-1 text-white bg-crossLightPurple w-100">
                <MdOutlineViewModule className="text-20px" /> Optional Modules
              </h4>
              <ul className="flex flex-col gap-0.5 list-disc w-100 items-start pl-3 py-1">
                {program &&
                  program.optionalModules.map((module, i) => (
                    <li key={i} className="">
                      {module}
                    </li>
                  ))}
          ...
          {program && program.notes[0] !== "" && (
            <div className="flex flex-col items-center justify-center h-auto bg-white w-100">
              <h4 className="flex gap-1 p-1 text-white bg-crossLightPurple w-100">
                <MdOutlineViewModule className="text-20px" /> Notes
              </h4>
              <ul className="flex flex-col gap-0.5 list-disc w-100 items-start pl-3 py-1">
                {program &&
        ...
        <div className="flex flex-col gap-3 border large:w-40 small:w-100">
          ...
          {program && program.objectives[0] !== "" && (
            <div className="flex flex-col items-center justify-center h-auto bg-white w-100">
              <h4 className="flex gap-1 p-1 text-white bg-crossLightPurple w-100">
                <GrWorkshop className="text-20px" /> Workshop Objectives
              </h4>
              <ul className="flex flex-col gap-0.5 list-disc w-100 items-start pl-3 py-1">
                {program &&
          ...
          {program && !program.category.toLowerCase().includes("open") && (
            <div className="flex flex-col items-center justify-center h-auto gap-2 pb-2 bg-white w-100">
              <h4 className="flex gap-1 p-1 text-white bg-crossLightPurple w-100">
                <LuAlarmClock className="text-20px" /> Duration
              </h4>
              <p className="pl-2 text-left w-100">
          ...
          <div className="flex flex-col gap-1 bg-white w-100">
            <h4 className="flex gap-1 p-1 text-white bg-crossLightPurple">
              <FaRegUser className="text-20px" /> Who Should Attend?
            </h4>
            <p className="leading-loose small:p-1">
              {program && program.targetAudience[0].toUpperCase()}
              {program && program.targetAudience.slice(1)}
            </p>
          </div>
        </div>
      </div>
      ...
      <div className="flex flex-col items-center gap-3 py-5 bg-white rounded large:w-83vw small:w-90vw">
        <div className="flex items-center w-auto h-auto">
          <button
            className={`flex items-center justify-center w-auto p-2  border h-40px ${messageMode === "individual" ? "bg-crossLightPurple text-white" : "border border-[#D9D9D9] text-crossTextGray"} rounded-tl-20 rounded-bl-20`}
          ...
          <button
            className={`flex items-center justify-center w-auto p-2 h-40px rounded-tr-20 rounded-br-20 ${messageMode === "corporate" ? "bg-crossLightPurple text-white" : "border border-[#D9D9D9] text-crossTextGray"}`}
        ...
        <div className="flex flex-col items-center h-auto small:gap-2 large:gap-3 w-100">
          <h5 className="font-bold large:text-20px small:text-15px">
            Schedule This Training
          </h5>
          ...
          {details === "course" && (
            ...
            <div className="flex flex-col items-center gap-2 large:px-2 w-100 small:px-1">
              <h4 className="self-start font-semibold large:text-20px text-crossLightPurple small:text-15px">
                Course Details
              </h4>
              <div className="flex flex-col items-center h-auto gap-2 w-100">
                <div className="flex items-center justify-between h-auto w-100">
                  <div
                    className={`flex flex-col h-auto ${executiveProgram ? "w-45" : "w-100"}`}
                  >
                    ...
                    <p className="text-vogueRed">
                      {enrollmentErrors && enrollmentErrors.program}
                    </p>
                  </div>
                ...
                <div className="flex items-center justify-between h-auto w-100">
                  ...
                  <div className="flex flex-col h-auto w-45">
                    ...
                    <select
                      ...
                      className="p-1 border rounded h-40px w-100"
                    >
                      <option value={program && program.duration}>
                        {program && program.duration}
                      </option>
                    </select>
                  </div>
              ...
              <button
                className="flex items-center justify-center text-white rounded w-100 h-40px bg-crossLightPurple"
                onClick={nextForm}
              >
                Next
              </button>
          ...
          {details === "personal" && (
            <div className="flex flex-col large:gap-4 large:px-2 w-100 small:px-1 small:gap-2">
              <FaArrowLeftLong
                className="cursor-pointer large:text-25px text-crossLightPurple small:text-20px"
              ...
              <div className="flex flex-col items-center h-auto gap-2 w-100">
                <div className="flex items-center justify-between h-auto w-100">
                  ...
                  <div className="flex flex-col h-auto w-45">
                    ...
                    <p className="text-vogueRed">
                      {enrollmentErrors && enrollmentErrors.fullName}
                    </p>
                  </div>
                </div>
