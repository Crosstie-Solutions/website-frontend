...
function LeadershipPage() {
  const { allMembers, fetchMembers } = useContext(CrossContext);
  ...
  const boardOfDirectors =
    allMembers &&
    allMembers
      .filter((member) => member.tier.trim().toLowerCase() === "directors")
  ...
  const boardOfAdvisory =
    ...
    allMembers
      .filter(
        (member) => member.tier.trim().toLowerCase() === "board of advisory"
      )
      .sort((a, b) => a.priorityIndex - b.priorityIndex);
  ...
  const teamMembers =
    allMembers &&
    allMembers
      .filter((member) => member.tier.trim().toLowerCase() === "team members")
      .sort((a, b) => a.priorityIndex - b.priorityIndex);

  const facultyMembers =
    allMembers &&
    allMembers
      .filter((member) => member.tier.trim().toLowerCase() === "our faculty")
      .sort((a, b) => a.priorityIndex - b.priorityIndex);
    ...
    <div className="relative flex flex-col items-center justify-start gap-5 pb-5 bg-white large:mt-12 text-15px large:w-100vw large:h-auto small:w-100vw small:h-auto small:mt-8">
      ...
      <div className="flex flex-col items-start h-auto gap-4 large:w-83vw small:w-90vw">
        <div className="flex flex-col items-start h-auto gap-1 w-100">
          <h2 className="font-bold text-left large:text-30px small:text-20px">
            The Crosstie Professionals
          </h2>
          <p className="text-left large:w-70">
            Crosstie professionals are industry veterans, passionate innovators,
            and certified experts, building excellence in education and customer
            service.
          </p>
        </div>
        ...
        <div className="flex flex-col items-center h-auto gap-5 w-100">
          ...
          <div className="flex flex-col items-center h-auto gap-2 w-100">
            <h4 className="py-1 pl-1 font-semibold text-white border-l-4 bg-crossTextGray text-20px w-100 border-crossYellow">
              Directors
            </h4>
          ...
          <div className="flex flex-col items-center h-auto gap-2 w-100">
            ...
            <p>
              Our faculty and consultant team is made up of seasoned
              professionals and certified facilitators recognised as the best in
              their respective professional fields. See below some of them:
            </p>
            ...
            <div className="flex flex-row flex-wrap justify-start h-auto large:gap-3 w-100 small:gap-5">
              {facultyMembers &&
                facultyMembers.map((member, i) => (
                  <div
                    className="flex flex-col h-auto pb-1 border-b-2 large:w-25 small:w-auto"
                    key={i}
                  >
                    <h3 className="font-semibold text-crossLightPurple">
                      {member.name}
                    </h3>
