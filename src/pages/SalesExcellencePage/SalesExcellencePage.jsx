...
function SalesExcellencePage() {
    ...
    const { programsSearchTerm, allPrograms, allCourses, downloadScreen, toggleDownloadScreen, courseBrochureDownloadScreen, downloadProgramScreen} = useContext(CrossContext);
  ...
  const title = allCourses && allCourses.length > 0 && allCourses[2].courseTitle;
    ...
    const location = window.location.pathname;
     ...
     const salesSeriesPrograms = allPrograms && allPrograms.filter(program=> program.category.toLowerCase().includes("sales"));
    
    
        // // Filter programs based on search term
      const filteredPrograms = salesSeriesPrograms && salesSeriesPrograms.filter((program) =>
  ...
  return (
    ...
    <AboutHero 
      ...
      buttonText="Download SES Course Brochure"
       line1= "Mastery. Impact. Performance-Driven"
